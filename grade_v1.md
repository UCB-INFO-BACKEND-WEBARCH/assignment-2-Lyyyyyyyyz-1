...
❌ **DOCK-03**: All services connect properly (app to db+redis, worker to redis)
   ...
   - Deduction: App crashed on startup due to missing depends_on healthcheck (DB not ready); required restart (-1 pts, minor)
...
- All services connect properly (app to db+redis, worker to redis): App crashed on startup due to missing depends_on healthcheck (DB not ready); required restart
